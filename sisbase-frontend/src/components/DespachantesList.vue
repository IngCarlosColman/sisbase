    ...
    <v-dialog v-model="dialog" max-width="600px">
      <v-card>
        <v-card-title class="text-h5">
          {{ isEditing ? 'Editar Despachante' : 'Agregar Despachante' }}
        </v-card-title>
        <v-card-text>
          <v-container>
...
const API_BASE_URL = 'http://localhost:8000/api/despachantes';
...
const fetchRecords = async () => {
  ...
  try {
    ...
    // Ordena toda la lista de registros cada vez que se carga un nuevo lote.
    records.value.sort((a, b) => {
      // Prioriza los registros con teléfono no vacío
      const aHasPhone = a.telefono && a.telefono.trim() !== '';
      const bHasPhone = b.telefono && b.telefono.trim() !== '';

      // Si 'a' tiene teléfono y 'b' no, 'a' va primero.
      if (aHasPhone && !bHasPhone) {
        return -1;
      }
      // Si 'b' tiene teléfono y 'a' no, 'b' va primero.
      if (!aHasPhone && bHasPhone) {
        return 1;
      }

      // Si ambos tienen o no tienen teléfono, ordena alfabéticamente por nombre
      // Esto proporciona un orden secundario consistente.
