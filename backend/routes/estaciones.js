//==========================================================================
// Archivo: estaciones.js
// Descripción: Define las rutas de la API para la tabla 'estaciones'.
...
router.post('/', async (req, res) => {
  const { razon_social, departamento, direccion, telefono } = req.body;
  try {
    const query = `
      INSERT INTO estaciones ("razon_social", "departamento", "direccion", "telefono")
      VALUES ($1, $2, $3, $4)
      RETURNING *;`;
    const values = [razon_social, departamento, direccion, telefono];
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (search && search.trim() !== '') {
      ...
      whereClause = `WHERE UPPER("razon_social") ILIKE $${paramIndex} OR UPPER("departamento") ILIKE $${paramIndex++}`;
    ...
    const finalQuery = `
      SELECT id, "razon_social", "departamento", "direccion", "telefono"
      FROM estaciones
      ${whereClause}
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const query = `
      UPDATE estaciones
      SET "razon_social" = $1, "departamento" = $2, "direccion" = $3, "telefono" = $4
      WHERE id = $5
      RETURNING *;`;
    const values = [razon_social, departamento, direccion, telefono, id];
    const result = await pool.query(query, values);
